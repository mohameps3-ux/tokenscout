...
const FREE_TOKEN_LIMIT = PLANS.FREE.tokens;   // 10
const FREE_PAGE_SIZE   = FREE_TOKEN_LIMIT;    // 10 per page for free
...
async function getTokens(
  ...
  // Free users always stay on page 1 (only top 10 allowed)
  ...
  // Free: cap at FREE_TOKEN_LIMIT regardless
  ...
  return {
    ...
    total,                          // true total (shown in upgrade prompt)
      ...
      <main className="flex-1 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="text-center py-5">
          <h1 className="text-3xl sm:text-4xl font-bold text-white tracking-tight">
            <span className="text-blue-400">Token</span>Scout 2.0
          </h1>
          <p className="mt-2 text-zinc-400 text-sm sm:text-base max-w-xl mx-auto">
            Real-time token scanner for Base & Solana. AI-scored for safety, liquidity & alpha potential.
          </p>
          <div className="flex flex-wrap justify-center gap-2 mt-4">
            {[
              ...
              { icon: Zap, text: "Multi-source Data" },
            ].map(({ icon: Icon, text }) => (
              <span
                key={text}
                className="flex items-center gap-1.5 text-xs text-zinc-400 bg-zinc-800/60 border border-zinc-700 rounded-full px-3 py-1"
              >
        ...
        {/* Filters (only for Pro in full mode; free users see them but can't paginate) */}
        ...
        {isEmpty && !hasNeverScanned ? (
          ...
          <>
            ...
            {/* Upgrade prompt when free user hits limit */}
        ...
        <footer className="border-t border-zinc-800 pt-6 pb-4 text-center">
          <p className="text-xs text-zinc-600 max-w-2xl mx-auto">
            <strong className="text-zinc-500">Disclaimer:</strong> TokenScout predictions and scores are for entertainment and research purposes only. This is{" "}
            <strong>not financial advice</strong>. Always do your own research before investing in any token. Crypto assets are highly volatile and you may lose all of your investment.
          </p>
        </footer>
      </main>
    </div>
