...
export function TokenDetailClient({ data, currentTimeframe }: TokenDetailClientProps) {
  ...
  const tradeLinks = chainCfg.tradeLinks(address, pairAddress);
  const listedDate = listedAt ? new Date(listedAt) : pairCreatedAt ? new Date(pairCreatedAt) : null;

  const aiScore = computeAiScore({
    liquidity,
    volume24h,
    pairCreatedAt: pairCreatedAt ?? (listedAt ? new Date(listedAt).getTime() : null),
    buys24h: txns24h?.buys ?? null,
    sells24h: txns24h?.sells ?? null,
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span
              className={`flex items-center gap-1 text-sm font-semibold ${aiScore.color}`}
              title={aiScore.reasons.join(" · ")}
            >
      ...
      <div className="flex gap-1 border-b border-zinc-800">
        {(["overview", "trade"] as const).map((tab) => (
          <button
            key={tab}
      ...
      {/* ── Trade tab ── */}
      {activeTab === "trade" && (
        <TradeWidget chain={chain} address={address} pairAddress={pairAddress} />
      )}
      ...
      {activeTab === "overview" && (
        <>
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-sm font-bold ${aiScore.color}`}>
                ...
                <div className="h-1.5 rounded-full bg-zinc-800 overflow-hidden">
                  <div
                    className={`h-1.5 rounded-full transition-all ${
                      aiScore.score >= 75 ? "bg-emerald-500" :
                      aiScore.score >= 50 ? "bg-yellow-500" :
                      aiScore.score >= 25 ? "bg-orange-500" : "bg-red-500"
                    }`}
                    style={{ width: `${aiScore.score}%` }}
                  />
                </div>
                <ul className="space-y-1">
                  {aiScore.reasons.map((r) => (
                    <li key={r} className="text-xs text-zinc-500 flex items-center gap-1.5">
          ...
          {/* Whale Activity */}
          <WhaleActivity chain={chain} address={address} pairAddress={pairAddress} />
