...
export function TokenCard({ token }: TokenCardProps) {
  const { label, bg } = getScoreLabel(token.totalScore);
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
        ...
        <Stat label="Age" value={formatAge(token.listedAt ? new Date(token.listedAt) : null)} />
      </div>
...
function getDexUrl(token: { chain: string; address: string; pairAddress: string | null; dexId: string | null }): string | null {
